...
# Instrumental variable approach using control function strategy with Martingale posterior
# First-stage: Standard least squares estimation
# Then: Estimate conditional density p(y | x, eta) where eta are first-stage residuals
# Finally: Integrate over empirical distribution of eta to obtain interventional density p(y(x))
def mp_density_iv(y, x, z, x_vals, y_grid, B_post, T_fwd, seed=42):
    """
    Estimate interventional distribution using instrumental variable control function approach.
    ...
    for x_val in x_vals:
        for eta_val in eta:
            ...
            x_target_list.append(np.full(n_y, x_val))
            eta_target_list.append(np.full(n_y, eta_val))
    ...
    _, logpdf_pr, _ = predictive_resample_cregression(
        fit, X_cov_jnp, y_target, X_target, B_post, T_fwd, seed=seed
    )
